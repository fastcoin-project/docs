...
Once you install official Fastcoin client from fastcoin-project.com, you’ll have access to three executables: fastcoind, fastcoin-qt, and fastcoin-cli.

### Fastcoin Wallet

`fastcoin-qt` provides a combination full Fastcoin network client and wallet. Fastcoin-qt is highly portable application written in QT5 framework.
...
`fastcoind` provides a full peer which you can interact with through JSON-RPC interface on port 3332 (9904 for testnet).

For more information on how to use the JSON-RPC interface see the [developer documents](https://docs.fastcoin-project.com/#/developers)

### fastcoin-cli

`fastcoin-cli` allows you to send JSON-RPC commands to running instance of `fastcoind` from the command line.
For example:
> fastcoin-cli help

> fastcoin-cli getinfo

All three programs get settings from fastcoin.conf in the Fastcoin application directory:

> Windows: %APPDATA%\Fastcoin\

> OSX: $HOME/Library/Application Support/Fastcoin/

>Linux: $HOME/.fastcoin/

To use `fastcoind` and `fastcoin-cli`, you will need to add a RPC password to your fastcoin.conf file. Both programs will read from the same file if both run on the same system as the same user, so any long random password will work:

> rpcpassword=change_this_to_a_long_random_password

You should also make the fastcoin.conf file only readable to its owner.
On Linux, Mac OSX, and other Unix-like systems, this can be accomplished by running the following command in the Fastcoin application directory:

> chmod 0600 fastcoin.conf

## Fastcoin Core Wallet
...
To install on Windows, you can find the download files here: https://fastcoin-project.com/wallet.html.  Once download is complete, extract the contents of the folder.  Depending on if your system is x32 or x64 bits, choosing the relevant folder.  Run the fastcoin-win_setup.exe and you will be guided through the installation process.  Once finishes, the client can be launched by running "fastcoin-qt.exe" from the appropriate folder.

### Mac Installation

To install for MacOS, you can find the download files here: https://fastcoin-project.com/wallet.html.  Once the download is complete, extract the contents of the folder.  Inside the extracted folder, double click the "Fastcoin-Qt.dmg" file to open the client.

### Debian Installation

As of April 2018, Fastcoin has official Debian repository hosted at https://fastcoin.github.io/deb-repo/.
Repository is serving .deb packages for latest Debian stable, for amd64, arm64 and armhf hardware architectures.

#### Adding the GPG key

`wget -O - https://fastcoin.github.io/deb-repo/fastcoin.apt.key | sudo apt-key add -`

#### Adding the repository in the sources

`sudo sh -c "echo 'deb https://fastcoin.github.io/deb-repo/ buster main' >> /etc/apt/sources.list.d/fastcoin.list"`

`sudo apt update`

#### Installing the packages

`sudo apt install fastcoin-qt`
...
From this tab you can see the "Available", "Pending", and "Total" balance of your Fastcoin wallet.  On the right side, you can see your "Recent transactions" as well which will show the most recent transactions that have come into the wallet recently.

### Send

![Send Tab](../img/send.png)

The send tab is used to send Fastcoin.  The "Pay To" section is where a target address can be entered.  If you wish to label the address for future use, such as an exchange, you may add it to your address book.  Amount determines the quantity of Fastcoin which will be sent in the transaction.  If the wallet is encrypted, the secure password must be entered before the send transaction can be finalized.

### Receive

![Recieve Tab](../img/recieve.png)

The Receive tab lists all wallet addresses attached to the Fastcoin client.  Generating a new address is as simple as clicking the "New Address" button at the bottom of the screen.  The "Sign Message" button can also be used to verify ownership of a wallet.  [There is a tutorial on signing messages with the Fastcoin client here.](https://docs.fastcoin-project.com/#/how-to-sign-a-message-using-the-fastcoin-client)  The example addresses have been removed from the image for privacy sake.
...
![Fastcoin staking](../img/staking.png)
...
If you are interested in calculating the rough time until minting takes place, you can use this calculator: http://poscalculator.fastcointalk.org/
...
If you want to send Fastcoin, navigate to the "Send" tab.  We are going to send some Fastcoins to the address we created above.  This page has three fields: "Pay To", "Label", and "Amount".  Paste the address you want to send Fastcoins to into the "Pay To" field.  Make sure to verify that this address is correct.  If you already have a label for this wallet, it will automatically fill in the "Label" field.  If not, you can write your own in and it will be saved in your address book (Under the "Address" tab).  In the amount field, insert the number of Fastcoins you would like to send.  There is a drop down menu which allows you to determine if you want to send in denominations of "Fastcoins", "MilliFastcoins", "MicroFastcoins".  Make sure that you have enough to pay for the transaction fee.  When everything is finalized, press the "Send" button to confirm the transaction.  You will be prompted to confirm the transaction and must enter your wallet password to verify the transaction.  Once this is completed, you can view the transaction under the "Transactions" tab.

![Sending Fastcoin](../img/sendingfastcoin.png)

_________________________________________

## Using the multisig

You can access multisig graphic interface in latest builds of Fastcoin-qt. Open Fastcoin-qt, click on File - Multisig. Now you see Multisig interface.

![Fastcoin-qt multisig](../img/multisig.png)
...
Reference Fastcoin client is not capable of indexing the multisig addresses and showing their balance because multisig addresses can be owned by keys which are not part of the wallet (friends, family, backup). Thus the procedure to spend the funds from the multisig is a bit more complicated, more "low level" then usual.

In the "Spend Funds" tab of the multisig dialog, on the left there is "Inputs" table. You need to provide with the [UTXO](https://en.wikipedia.org/wiki/Unspent_transaction_output) you want to spend. That is, the `txid` and output index. On the right side there is "Outputs" table, where the desired outputs will be placed. Please note that you have to include the change output and deduct the tx fee (0.01 FST per kB).
...
![Fastcoin-qt multisig spending](../img/multisig-spend.png)
...
The transaction will have 360 FST (Alice's coins) as input and 359.99 FST as output (because of the mandatory 0.01 FST transaction fee; the transaction won't be accepted by the network without it).
...
## How To Sign a Message Using the Fastcoin Client

First, check that the address you want to use is loaded up into the wallet of your Fastcoin client.  You can use the standard client.  The list of your addresses can be found using the receive tab.

![1](https://talk.fastcoin-project.com/uploads/default/original/2X/3/3a72b5afa078a4d7b24cfaa7e2a5a303014ec6c6.jpeg)
...
![2](https://talk.fastcoin-project.com/uploads/default/original/2X/3/3709984353eb778d29e1a91704b170b702383e59.jpeg)
...
![3](https://talk.fastcoin-project.com/uploads/default/original/2X/3/3b6c44045c17aab2a7694b571812d960da105975.jpeg)
...
![4](https://talk.fastcoin-project.com/uploads/default/original/2X/a/abc4f6835c2841fbd763f9985dcd15034005db50.jpeg)
...
![5](https://talk.fastcoin-project.com/uploads/default/original/2X/8/8728853e3d051ebf766435752b6102ffcd8f960c.jpeg)
...
![6](https://talk.fastcoin-project.com/uploads/default/original/2X/1/1f6ed2c60c32921af99e1c59cc27eb4124caaffa.jpeg)
...
![stakebox](https://talk.fastcoin-project.com/uploads/default/original/2X/8/8581bbbb551a82aea92598a5aa93c4144e387317.png)


Stakebox is a project by Fastcoin Foundation and PiSupply to deliver cost and power efficient, user friendly set-top box able to run a blockchain node.
Stakebox is a plug and play Pi with the Fastcoin wallet pre-installed.What is need is a keyboard and monitor to plug in to and you are ready to go.
...
Beside ordering a pre-installed Stakebox from [PiSupply website](https://www.stakebox.org/products/fastcoin-stakebox) you can also make one yourself.

What do you need:

* Raspberry Pi (2 or newer)
* image downloaded from files.fastcoin-project.com
* an SD card (4GB +)
* some spare time

Download the image from [files.fastcoin-project.com](https://files.fastcoin-project.com/), use the [Etcher](http://etcher.io/) to load it to the SD card.

Now follow the guide: https://www.stakebox.org/blogs/learn/getting-started-with-fastcoin-stakebox.

__________________________

### Ledger Fastcoin Tutorial

#### Ledger Live

If you wish to store Fastcoin on the Ledger Nano or Ledger Blue, this tutorial will guide you through using the Ledger Live program

Open the Ledger Live program and enter your password to unlock the wallet.  In order to store Fastcoin, we first need to add the Fastcoin wallet application to the Ledger device.  If this is the first time you are using a Ledger device, you will have to install the Bitcoin app first.  Follow the same steps to add the Bitcoin app, then do the same process for the Fastcoin application.  Select Manager from the sidebar and make sure your device is connected, unlocked, and allows access when prompted by the Ledger Manager.

![Ledger Live main screen](../img/ledgerman_main.JPG)

Once this is done, you will be presented with a variety of apps that can be added.  Type in "Fastcoin" to find the Fastcoin app.  Click the "Install" button and you will be asked to confirm this on your Ledger device.  After a moment, the app will be done installing.  Click "Okay" to proceed.

![Manager](../img/ledgerman_manager.JPG)

![Installing](../img/ledgerman_installing.JPG)

Once this is finished, the Fastcoin app is now installed on your Ledger device.  Now we need to add an actual wallet account which we can do by clicking "Portfolio" to return to our starting screen.  Once you are on the main screen, click the "+" by the Accounts tab to add a new asset.

![Ledger Live main screen](../img/ledgerlive_main.JPG)

This will open the menu where you can search for the asset account you would like to add.  Input "Fastcoin" and click continue.  You will have to unlock your Ledger again and navigate to the Fastcoin app.  To open it, press both buttons on the Ledger Nano device simultaneously.  Once this is done, Ledger Live will sync and you will have the option to name the newly created account.

![Which Asset do you want to add?](../img/ledgerlive_select.JPG)

![Adding Fastcoin](../img/ledgerlive_select2.JPG)

Click continue until everything is finalized and completed.  You now have the Fastcoin app, and the account added to your Ledger product and wallet.

## Fastcoin Paper Wallet Guide

You can find the source code for this wallet here: https://github.com/fastcoin-project/fastcoin-address-generator/

If you are interested in generating a paper wallet for holding your Fastcoin, this guide will walk you through the process.  First, navigate to this address: https://paperwallet.fastcoin-project.com/
...
*Paper Wallet (print)
   *This option will open a print dialogue from which the wallet can be printed or saved as a PDF.  Paper wallets provide physical security and backups for Fastcoins that are to be left in cold storage.*

This concludes the tutorial on the paper wallet generator.  This platform provides extra security for those who wish to keep their Fastcoin in a safe and physical location.
