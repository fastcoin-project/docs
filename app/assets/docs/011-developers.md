...
As Fastcoin is made to run on range of platforms, from Amazon's server to low powered Raspberry Pi Debian is perfect OS platform for deploying Fastcoin nodes as it is renowned for multitude of supported hardware architectures as well as security and stability.
...
Download the latest .tar.gz from github.com/fastcoin-project.

> wget https://github.com/fastcoin-project/fastcoin/releases/download/v0.11.0sum/fastcoin-0.11.0.tar.gz

Debian build system is very strict about names, so we need to rename this to:

`fastcoin_0.11.0.orig.tar.gz`
...
Fastcoin daemon offers JSON-RPC interface which can be used to control the daemon or integrate it with software stack.
You can send commands to the daemon by using `fastcoin-cli` tool.

There are two official wrappers for this interface, a PHP one and a Python2.7+ one.

> Fastcoin_rpc is a simple and minimal library made for communication with fastcoind via JSON-RPC protocol. It has a single dependency - a Python requests library and it supports both mainnet and testnet fastcoin network with authentication or SSL encryption. There is a single class to be imported from the library - Client.

https://github.com/fastcoin-project/fastcoin_rpc

> fastcoin-php-rpc is a simple and minimal library made for communication with fastcoind via JSON-RPC protocol for PHP 7.1+. Easiest way to use is to use composer. Otherwise include RpcClient class in your project an you are good to go.

https://github.com/fastcoin-project/fastcoin-php-rpc
...
| `sendfrom`         | `fromaccount` `tofastcoinaddress` `amount` [minconf=1] [comment] [comment-to] |`amount` is a real and is rounded to 6 decimal places. Will send the given amount to the given address, ensuring the account has a valid balance using [minconf] confirmations. Returns the transaction ID if successful (not in JSON object).|yes|
...
## Fastcoin Developer Notes

Constants that may be useful when looking to integrate / develop with Fastcoin.

Fastcoin source code repository: github.com/fastcoin-project/fastcoin
...
Fastcoin transaction format is indendical to a Bitcoin transaction format with the exception of included transaction timestamp.
...
As of Fastcoin 0.11, timestamp is no longer required and transaction format is exactly the same as Bitcoin. The only exception is that `version` is now `3`.
...
In this simple example it it will be demonstrated how to use popular bitcore library to create a Fastcoin transaction.
...
Using bitcore is possible because Fastcoin is based of Bitcoin and the two share more than 99% of the code.

```
const bitcore = require('bitcore-lib');

//
// Add fastcoin network params
//
...
// set fastcoin-testnet as default network
bitcore.Networks.defaultNetwork = bitcore.Networks.get('fastcoin-testnet');
...
console.log("This is my address: ", myAddress.toString()); // this is the address which will be spending coins, so you need testnet fastcoins sent here. Use faucet: https://fastcoinexplorer.net/faucet/
...
const transaction = new bitcore.Transaction()
    ...
    .from(utxo)
    .feePerKb(10000) // data on Fastcoin costs 0.01 FST / kB
    .to(reciever, 1) // sending 1 tFST to reciever
    .addData("my test transaction!") // Add transaction metadata
    .change(myAddress) // change
    .sign(privateKey);

// Overriding txn version to 3, because that's what it needs to be to work with fastcoin
...
We've put some recent copies on our [file server](https://files.fastcoin-project.com) :)

* [Mainnet bootstrap.dat.zip](https://files.fastcoin-project.com/download/fastcoin_mainnet_2018_08_06_bootstrap.dat.zip) Scrypt `66c494e0c2a4c78ba19f14a3781ca77f1b8b16f3833fb85c10959ee991764a4c` | [torrent](https://files.fastcoin-project.com/download/fastcoin_mainnet_2018_08_06_bootstrap.dat.zip.torrent) | [magnet](magnet:?xt=urn:btih:9fc1f0d09a6598ae96ba5d8b9ac5caca0ae92402&dn=fastcoin%5Fmainnet%5F2018%5F08%5F06%5Fbootstrap.dat.zip&tr=udp%3A%2F%2Fpublic.popcorn-tracker.org%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.ilibr.org%3A80%2Fannounce&tr=http%3A%2F%2Fatrack.pow7.com%2Fannounce&tr=http%3A%2F%2Fbt.henbt.com%3A2710%2Fannounce&tr=http%3A%2F%2Fmgtracker.org%3A2710%2Fannounce&tr=http%3A%2F%2Fmgtracker.org%3A6969%2Fannounce&tr=http%3A%2F%2Fopen.touki.ru%2Fannounce.php&tr=http%3A%2F%2Fp4p.arenabg.ch%3A1337%2Fannounce&tr=http%3A%2F%2Fpow7.com%3A80%2Fannounce&tr=http%3A%2F%2Fretracker.krs-ix.ru%3A80%2Fannounce)
* [Mainnet bootstrap.dat.tar.gz](https://files.fastcoin-project.com/download/fastcoin_mainnet_2018_08_06_bootstrap.dat.tar.gz) Scrypt `bac807186735347e2c7ccbfecb3d91045de32246258a7ba3e3d0a9c2a10b8ff0` | [torrent](https://files.fastcoin-project.com/download/fastcoin_mainnet_2018_08_06_bootstrap.dat.tar.gz.torrent) | [magnet](magnet:?xt=urn:btih:77f1c5352e9eca71e9f4f1e31487e7d13900a335&dn=fastcoin%5Fmainnet%5F2018%5F08%5F06%5Fbootstrap.dat.tar.gz&tr=udp%3A%2F%2Fpublic.popcorn-tracker.org%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.ilibr.org%3A80%2Fannounce&tr=http%3A%2F%2Fatrack.pow7.com%2Fannounce&tr=http%3A%2F%2Fbt.henbt.com%3A2710%2Fannounce&tr=http%3A%2F%2Fmgtracker.org%3A2710%2Fannounce&tr=http%3A%2F%2Fmgtracker.org%3A6969%2Fannounce&tr=http%3A%2F%2Fopen.touki.ru%2Fannounce.php&tr=http%3A%2F%2Fp4p.arenabg.ch%3A1337%2Fannounce&tr=http%3A%2F%2Fpow7.com%3A80%2Fannounce&tr=http%3A%2F%2Fretracker.krs-ix.ru%3A80%2Fannounce)
* [Testnet bootstrap.dat.zip](https://files.fastcoin-project.com/download/fastcoin_testnet_2018_08_06_bootstrap.dat.zip) Scrypt `1312aaaf0d8466b6f7006bac60a5cad4daf36e1241f791924e45bc5959ff2452` | [torrent](https://files.fastcoin-project.com/download/fastcoin_testnet_2018_08_06_bootstrap.dat.zip.torrent) | [magnet](magnet:?xt=urn:btih:c70afb5100953362b45df75133e01bf1f9466e04&dn=fastcoin%5Ftestnet%5F2018%5F08%5F06%5Fbootstrap.dat.zip&tr=udp%3A%2F%2Fpublic.popcorn-tracker.org%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.ilibr.org%3A80%2Fannounce&tr=http%3A%2F%2Fatrack.pow7.com%2Fannounce&tr=http%3A%2F%2Fbt.henbt.com%3A2710%2Fannounce&tr=http%3A%2F%2Fmgtracker.org%3A2710%2Fannounce&tr=http%3A%2F%2Fmgtracker.org%3A6969%2Fannounce&tr=http%3A%2F%2Fopen.touki.ru%2Fannounce.php&tr=http%3A%2F%2Fp4p.arenabg.ch%3A1337%2Fannounce&tr=http%3A%2F%2Fpow7.com%3A80%2Fannounce&tr=http%3A%2F%2Fretracker.krs-ix.ru%3A80%2Fannounce)
* [Testnet bootstrap.dat.tar.gz](https://files.fastcoin-project.com/download/fastcoin_testnet_2018_08_06_bootstrap.dat.tar.gz) Scrypt `15f0e40a7e99083b2ed27c78d37d0f201fd6c744537cf643925d0aae84395896` | [torrent](https://files.fastcoin-project.com/download/fastcoin_testnet_2018_08_06_bootstrap.dat.tar.gz.torrent) | [magnet](magnet:?xt=urn:btih:eb2a70e90285ca847be47b2f96e36e5cdec4580e&dn=fastcoin%5Ftestnet%5F2018%5F08%5F06%5Fbootstrap.dat.tar.gz&tr=udp%3A%2F%2Fpublic.popcorn-tracker.org%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.ilibr.org%3A80%2Fannounce&tr=http%3A%2F%2Fatrack.pow7.com%2Fannounce&tr=http%3A%2F%2Fbt.henbt.com%3A2710%2Fannounce&tr=http%3A%2F%2Fmgtracker.org%3A2710%2Fannounce&tr=http%3A%2F%2Fmgtracker.org%3A6969%2Fannounce&tr=http%3A%2F%2Fopen.touki.ru%2Fannounce.php&tr=http%3A%2F%2Fp4p.arenabg.ch%3A1337%2Fannounce&tr=http%3A%2F%2Fpow7.com%3A80%2Fannounce&tr=http%3A%2F%2Fretracker.krs-ix.ru%3A80%2Fannounce)
